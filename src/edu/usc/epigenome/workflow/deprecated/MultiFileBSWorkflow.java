...
//			{
//				if(workFlowParams.getSetting("Lane." + i + ".AlignmentType").toLowerCase().contains("fastbs"))
//				{
...
//					int splitSize = Integer.parseInt(workFlowParams.getSetting("ClusterSize")) / workFlowParams.getAvailableLanes().length;
...
//						dax.addChild(fastq2bfqJob.getID(), isPE ? fastqSplitJob.getID() : filterContamJob.getID());
...
//									"bisulfite", Integer.parseInt(workFlowParams.getSetting("MaqTrimEnd1")), Integer.parseInt(workFlowParams.getSetting("MaqTrimEnd2")));
//							dax.addJob(mapJob);
