...
my $MIN_QUAL = 0;
my $MAX_IDENT = 1;
my $USAGE = "generateNmerInfo_fastq.pl [-test] [6] [10] [12] file1.fastq file2.fastq ...";
...
FILE: foreach my $fn (@fns)
{
	...
	# wig file
	push(@cmds, "fastqToFasta.pl < ${fn} | java -Xmx1995m edu.usc.epigenome.scripts.FastaToNmerCounts -nmer ${wind} > ${outfn}");
    }
